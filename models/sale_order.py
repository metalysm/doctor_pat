...
class SaleOrder(models.Model):
    ...
    invoice_ids = fields.One2many('account.move', 'appointment_id', compute="_compute_invoice_count")
    ...
    def _compute_invoice_ids(self):
        for rec in self:
            ...
            print('invoice ids 1: ', rec.invoice_ids)
    ...
    def _compute_invoice_count(self):
        for rec in self:
            rec.invoice_ids = rec.env['account.move'].search([('appointment_id', '=', rec.appointment_id.id), ('move_type', '=', 'out_invoice')])
            print('invoice ids 2: ', rec.invoice_ids)
            rec.invoice_count = len(rec.invoice_ids)
    ...
    def action_open_invoice(self):
        invoices = self.env['account.move'].search([
            ('appointment_id', '=', self.appointment_id.id), ('move_type', '=', 'out_invoice')
